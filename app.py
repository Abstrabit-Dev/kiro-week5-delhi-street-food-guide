...
def clean_agent_output(output):
    ...
    clean_output = re.sub(r'\[kiro-week5-delhi-street-food-guide\].*?>', '', clean_output)  # Remove agent prompt
...
def call_kiro_agent(user_message):
    ...
    try:
        ...
        result = subprocess.run(
            ["kiro-cli", "chat", "--agent", "kiro-week5-delhi-street-food-guide", "--no-interactive", user_message],
            capture_output=True,
            text=True,
            cwd=os.getcwd(),
            timeout=30
